...
inline glm::vec3 rotate(glm::vec3 point, float radians) {
	glm::vec3 temp(0, 0, 0);
...
//inline void rotate(glm::vec2& point, float radians) {
//	glm::vec2 temp;
//	temp.x = (point.x * cos(radians)) + (point.y * -sin(radians));
//	temp.y = (point.x * sin(radians)) + (point.y * cos(radians));
//	point = temp;
...
inline glm::vec2 rotate(glm::vec2 point, float radians) {
	glm::vec2 temp;
	temp.x = (point.x * cos(radians)) + (point.y * -sin(radians));
	temp.y = (point.x * sin(radians)) + (point.y * cos(radians));
	return temp;
...
inline glm::vec3 rotate(glm::vec3 point, glm::vec3 angle) {
	glm::vec3 temp;
...
inline float magnitude(glm::vec3 v) {
	return (float)sqrt(v.x * v.x + v.y * v.y + v.z * v.z);
}

inline float angleBetween(glm::vec3 A, glm::vec3 B) {
...
inline void clip(float num, float min, float max) {
	if (num < min) {
		num = min;
		return;
	} else if (num > max) {
...
inline void print(glm::vec3 a) {
	std::cout << "(" << a.x << ", " << a.y << ", " << a.z << ")";
}
inline void println(glm::vec3 a) {
	std::cout << "(" << a.x << ", " << a.y << ", " << a.z << ")\n";
}
inline void print(glm::ivec3 a) { print((glm::vec3)a); }
inline void println(glm::ivec3 a) { println((glm::vec3)a); }

inline void println(glm::vec2 a) {
	std::cout << "(" << a.x << ", " << a.y << ")\n";
}

inline void print(glm::vec2 a) {
	std::cout << "(" << a.x << ", " << a.y << ") ";
}
...
inline void printMat4Linear(glm::mat4 m) {
	std::cout 
		<< m[0][0] << ", " << m[1][0] << ", " << m[2][0] << ", "
		<< m[0][1] << ", " << m[1][1] << ", " << m[2][1] << ", "
		<< m[0][2] << ", " << m[1][2] << ", " << m[2][2] << "\n";
...
inline float distToLineSeg(glm::vec2 p, glm::vec2 a, glm::vec2 b, glm::vec2 *closestPoint) {
	glm::vec2 ab = b - a;
	glm::vec2 ap = p - a;
...
inline bool isLeft(glm::vec2 point, glm::vec2 lineSegPoint1, glm::vec2 lineSegPoint2) {
...
inline bool point_in_polygon(glm::vec2 point, std::vector<glm::vec2> polygon) {
...
inline float getVecAngle(glm::vec2 vec) {
...
inline float cross(glm::vec2 a, glm::vec2 b) { return a.x * b.y - b.x * a.y; }

// Calculates the general line formula (ax + by + c = 0) from two points (p1 and p2) 
// and stores them in the a, b, and c inputs.
inline void generalLineFormula(glm::vec2 p1, glm::vec2 p2, float &a, float &b, float &c) {
	a = p1.y - p2.y;
	b = p1.x - p2.x;
...
inline bool whatSide(glm::vec2 lsp1, glm::vec2 lsp2, glm::vec2 p) {
...
inline glm::vec2 intersection(glm::vec2 A, glm::vec2 B, glm::vec2 C, glm::vec2 D) {
...
inline int orientation(glm::ivec2 p, glm::ivec2 q, glm::ivec2 r) {
...
inline bool cropFrustumToFrustum(std::vector<glm::vec2> &subjectPoly,
...
// Crops subjectPoly to cropToPoly.  Both need to be wound clockwise!
inline bool cropTileToFrustum(std::vector<glm::vec2> &subjectPolyVerts,
...
// Crops subjectPoly to cropToPoly.  Both need to be wound clockwise!
inline bool sutherlandHodgemanPolyCrop(std::vector<glm::vec2> &subjectPoly, std::vector<glm::vec2> cropToPoly,
...
inline std::vector<glm::vec2> sutherlandHodgemanLineCrop(std::vector<glm::vec2> subjectPoly, std::vector<glm::vec2> *cropToLine,
														 bool removeDuplicatePoints) {
...
inline float angleBetween(glm::vec2 A, glm::vec2 B) {
